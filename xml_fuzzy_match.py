...
def get_bounds(checkpoint_xml_path, node_id):
    ...
    checkpoint_json_path = os.path.join(dir_path, checkpoint_xml_path.split("/")[-1].split(".")[0]+".json")
...
def get_xml_fuzzy_match(checkpoint_xml_path,node_id,captured_xml_path,COSSINE_BOUND=0.65):
    """"
    ...
    cosine_similarity, status = check_sentence_similarity(simp_tree1, simp_tree2, threshold=COSSINE_BOUND)
    # print(f"similarity: {similarity}")
    logging.info(f"fuzzy match: {checkpoint_xml_path} vs {captured_xml_path}: cosine_similarity: {cosine_similarity}")
