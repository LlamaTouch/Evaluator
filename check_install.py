...
def check_install(checkpoint_installed_ls,captured_dir):
    ...
    cnt = 0
    app_package = pd.read_csv("/data/jxq/mobile-agent/comparison_algorithm/app_package_map.csv")
    ...
    for i in range(len(checkpoint_installed_ls)):
        if app_package_map[checkpoint_installed_ls[i].node_id] in captured_installed_packages:
            cnt += 1
...
def check_uninstall(checkpoint_installed_ls,captured_dir):
    ...
    for i in range(len(checkpoint_installed_ls)):
        if app_package_map[checkpoint_installed_ls[i].node_id] not in captured_installed_packages:
...
if __name__ == "__main__":
    ...
    ls = [Checkpoint(1, "check_uninstall", "ebay",)]
