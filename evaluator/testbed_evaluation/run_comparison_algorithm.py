...
dir_map["web_shopping"] = (
    f"/data/zzh/mobile-agent/Auto-UI/agentenv/agent_result/{classes}/"
)
...
dir_map["googleapps"] = (
    f"/data/zzh/mobile-agent/Auto-UI/agentenv/agent_result/google_apps/"
)
...
for i, row in instruction_data.iterrows():
    ...
    if comparison_algorithm(checkpoint_dir, captured_dir, COSINE_BOUND):
        ...
        logging.info(
            f"comparison_algorithm failed: {checkpoint_dir} and {captured_dir}"
        )
...
for index, row in instruction_data.iterrows():
    output_data.loc[index] = [
        str(row["episode_id"]),
        row["instruction"],
        testbed_judge_success_state[index],
    ]
...
logging.info(
    f"testbed judge success_rate: {testbed_judge_success_state.count(True)/total_instruction_num}"
)
